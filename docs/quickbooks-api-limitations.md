...
QBO supports querying attachment IDs by `AttachableRef.EntityRef.Type` and `.value`. These query results are sparse, so `list_transaction_attachables` performs bounded full reads before returning allowlisted metadata. Both `get_attachable` and attachment lists intentionally exclude temporary signed URLs; `read_attachable_content` consumes them only inside the server.
