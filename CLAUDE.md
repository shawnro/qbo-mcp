...
- `read_attachable_content` downloads only QBO-issued URLs: bounded UTF-8 text/CSV/XML (256 KB), JPEG/PNG/GIF images and embedded PDFs (10 MB stdio, 4 MB HTTP/Lambda). It refreshes expired URLs once and never accepts arbitrary URLs. HTTP metadata lists are capped at 20.
